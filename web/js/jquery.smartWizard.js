...
(function ($) {
    $.fn.smartWizard = function (action) {
        ...
        return this.each(function () {
            ...
            var contentWidth = 0;
            var loader, msgBox, elmActionBar, elmStepContainer, btNext, btPrevious, btCancel, btFinish;
            ...
            function init() {
                ...
                if(options.nextClass) {
                    btNext = $('<a>' + options.labelNext + '</a>').attr("href", "#").addClass(options.nextClass);
                } else {
                    btNext = $('<a>' + options.labelNext + '</a>').attr("href", "#").addClass("buttonFinish");
                }
                if(options.previousClass){
                    btPrevious = $('<a>' + options.labelPrevious + '</a>').attr("href", "#").addClass(options.previousClass);
                } else {
                    btPrevious = $('<a>' + options.labelPrevious + '</a>').attr("href", "#").addClass("buttonFinish");
                }
                if(options.cancelClass) {
                    btCancel = $('<a>' + options.labelCancel + '</a>').attr("href", "#").addClass(options.cancelClass);
                } else {
                    btCancel = $('<a>' + options.labelCancel + '</a>').attr("href", "#").addClass("buttonFinish");
                }
                if(options.finishClass) {
                    btFinish = $('<a>' + options.labelFinish + '</a>').attr("href", "#").addClass(options.finishClass);
                } else {
                ...
                $(btCancel).click(function () {
                    if(curStepIdx == options.selected) {
                        return false;
                    }
                    for(var i=options.selected; i<steps.length; i++) {
                        steps.eq(i).removeClass("done").addClass("disabled").removeAttr("isdone");
                    }
                    showStep(options.selected);
                    return false;
                });
            ...
            function SetupStep(curStep, selStep) {
                ...
                if($(curStep, obj).hasClass("disabled")) {
                    $(curStep, obj).removeClass("done");
                } else {
                ...
                if ($.isFunction(options.onShowStep)) {
                    if (!options.onShowStep.call(this, $(selStep), $(curStep))) {
                        return false;
                    }
                }
            }
            ...
            function adjustButton() {
                ...
                // Finish Button
                if ((!steps.hasClass('disabled')&&curStepIdx == (steps.length-1)) || options.enableFinishButton) {
                    $(btFinish).removeClass("buttonDisabled");
                } else {
                    $(btFinish).addClass("buttonDisabled");
                }
                if (!steps.hasClass('disabled') || options.enableCancelButton) {
                    $(btCancel).removeClass("buttonDisabled");
                } else {
                    $(btCancel).addClass("buttonDisabled");
                }
            }
